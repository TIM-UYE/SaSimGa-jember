...
@section('content')
                        ...
                        <div class="p-5">
                            ...
                            <div class="flex items-center justify-between gap-3">
                                ...
                                <div class="flex gap-2">
                                    ...
                                    <button type="button" onclick='openMenuDetail(@json($menu))'
            ...
            @if ($specials->isEmpty())
                <div class="rounded-3xl border border-orange-500 bg-orange-950/10 p-10 text-center text-orange-200">
                    ...
                    <p class="mt-2 text-sm text-orange-300">
                        Tambah menu specials di panel admin untuk menampilkan paket spesial di halaman ini.
                    </p>
                </div>
            @else
    ...
    <div id="specialMenuModal" onclick="closeSpecialModal(event)"
        ...
        <div class="bg-gradient-to-b from-gray-900 to-black rounded-3xl w-full max-w-6xl border border-gray-800 overflow-hidden"
            ...
            <div
                class="sticky top-0 z-20 bg-gray-900/95 backdrop-blur-sm border-b border-gray-800 px-6 py-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                <div>
            ...
            <div class="grid md:grid-cols-2">
                ...
                <div class="p-8 text-white">
                    ...
                    <button id="specialOrderBtn" type="button" disabled
                        class="w-full bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 py-4 rounded-2xl font-bold transition-all hover:scale-[1.02] opacity-50 cursor-not-allowed">
                        Pesan Sekarang
                    </button>
                </div>
            </div>
    ...
    <div id="floatingCheckout" class="hidden fixed bottom-5 right-5 z-40">
        <a href="{{ route('checkout.index') }}"
            class="group flex items-center gap-3 bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white px-4 py-3 rounded-2xl shadow-2xl shadow-orange-500/30 transition-all duration-300 hover:scale-105">
            <div
                class="w-10 h-10 rounded-xl bg-white/10 border border-white/10 flex items-center justify-center shrink-0">
                <i class="fas fa-shopping-cart text-base text-white"></i>
            </div>

            <div class="leading-tight">
                <p class="text-[10px] text-white/80 font-medium">Total</p>
                <p id="floatingTotal" class="text-lg font-bold text-white">Rp 0</p>
            </div>

            <div class="flex items-center gap-1 ml-1 shrink-0">
                <span class="font-semibold text-sm text-white">Checkout</span>
    ...
    <!-- CART TOAST -->
    <div id="cartToast"
        class="fixed top-24 right-6 z-[9999] hidden bg-zinc-900 border border-orange-500/40 text-white px-5 py-4 rounded-2xl shadow-2xl shadow-orange-500/20">
        <div class="flex items-center gap-3">
            <div class="w-10 h-10 rounded-full bg-orange-500 flex items-center justify-center animate-cart-bounce">
                <i class="fas fa-cart-shopping text-white"></i>
            </div>

            <div>
                <p class="font-bold">Berhasil ditambahkan</p>
                <p class="text-sm text-gray-400">Menu masuk ke keranjang</p>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        let currentSpecialItemId = null;

        const specialItemAddBaseUrl = "{{ url('/cart/special-item/add') }}";

        function addCartWithAjax(url, qty = 1) {

            const toast = document.getElementById('cartToast');

            if (toast) {
                toast.classList.remove('hidden', 'cart-toast-hide');
            ...
            .catch(() => {
                if (toast) {
                    toast.classList.add('hidden');
                    toast.classList.remove('cart-toast-show', 'cart-toast-hide');
                }
        ...
        function quickAddToCart(menuId) {
            addCartWithAjax('/cart/add/' + menuId, 1);
        }
        ...
        function addToCartFromModal() {
            ...
            const qty = document.getElementById('modalQtyInput').value || 1;
            addCartWithAjax('/cart/add/' + currentModalMenuId, qty);
        }
        ...
        document.addEventListener('visibilitychange', function() {
            if (!document.hidden) updateFloatingCheckout();
        });
        ...
        function showMenuSection(type) {
            ...
            window.scrollTo({
                top: document.getElementById(type === 'regular' ? 'regularMenuSection' : 'specialMenuSection')
                    .offsetTop - 100,
                behavior: 'smooth'
            });
        }
        ...
        function openSpecialModal(id) {
            const special = specialData.find(specialItem => Number(specialItem.id) === Number(id));
            if (!special) return;

            const actions = document.getElementById('specialItemButtons');
            actions.innerHTML = '';

            if (!special.items || special.items.length === 0) {
                selectSpecialItem(special, null);
            } else {
                special.items.forEach((item, index) => {
                    const button = document.createElement('button');
                    button.type = 'button';
                    button.className =
                        'special-item-btn w-full text-left bg-gray-800 hover:bg-orange-500 p-5 rounded-2xl transition-all text-white';
                    button.textContent = item.name;
                    button.dataset.itemId = item.id;

                    button.addEventListener('click', () => {
                        document.querySelectorAll('.special-item-btn').forEach(btn => {
                        ...
                        selectSpecialItem(special, item);
                    });
        ...
        function closeSpecialModal(event) {
            ...
            document.body.style.overflow = 'auto';
            currentSpecialItemId = null;
        }

        function selectSpecialItem(special, item) {
            const orderBtn = document.getElementById('specialOrderBtn');
            ...
            currentSpecialItemId = Number(item.id);
            ...
            if (orderBtn) {
                ...
                orderBtn.onclick = function() {
                    addSpecialItemToCart(Number(item.id));
                };
            }
        }

        function addSpecialItemToCart(itemId) {
            const selectedId = Number(itemId || currentSpecialItemId);

            if (!selectedId) {
                alert('Pilih varian menu spesial terlebih dahulu.');
                return;
            }

            addCartWithAjax(`${specialItemAddBaseUrl}/${selectedId}`, 1);
        }
    ...
    <style>
        .cart-toast-show {
            animation: cartToastIn 0.45s ease-out forwards;
        }

        .cart-toast-hide {
            animation: cartToastOut 0.35s ease-in forwards;
        }
